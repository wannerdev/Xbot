...
public class MinMax {
	...
	public int maxN(int player, int depth, int alpha) throws Exception {
		...
		for (Move move : posMoves) {
			...
			if (ratings[secondPl] == Integer.MAX_VALUE || ratings[thirdPl] == Integer.MAX_VALUE
					|| ratings[fourthPl] == Integer.MAX_VALUE) {
...
/*
 * Shallow(Node, Player, Bound) IF Node is terminal, RETURN static value Best =
 * Shallow(first Child, next Player, Sum) FOR each remaining Child IF
 * Best[Player] > = Bound, RETURN Best Current = Shallow(Child, next Player, Sum
 * - Best[Player]) IF Current[Player] > Best[Player], Best = Current RETURN Best
 * 
 */
/**
